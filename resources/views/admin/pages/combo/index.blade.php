@extends('admin.admin')
@section('navbar_title', 'Quản lý combo')
@section('navbar-right')
    <li>
        <a href="{{ route('admin_combo/form') }}" style="padding:5px 5px">
            <button class="btn bg-info heading-btn" type="button">Tạo combo</button>
        </a>
    </li>
@endsection
@section('content')
    <div class="panel panel-flat">
        <table class="table table-xlg datatable-ajax" data-source="{{ route('admin_combo/dataList') }}"
            data-destroymulti="{{ route('admin_combo/destroyMulti') }}">
            <thead>
                <tr>
        ...
        if ($('.datatable-ajax').length) {
            var columnDatas = [{
               ...
               
                {
                    ...
                    class: "text-right",
                    render: function(data) {
                        var $element = `<a href="javascript:void(0);" class="price" data-id="${data.id}" data-url="${base_domain}/admin/product/updateField/${data.id}" data-price="${data.price}" data-price_sale="${data.sale_price }">
                        `;
